...
function LeadModal({item,mode,onClose,onSave,onDelete,onConvert,canEdit,t}){
 ...
 return <div className="modal-backdrop"><section className="partner-modal lead-modal">
  ...
  {editing?<form className="partner-form" onSubmit={submit}>
   ...
   <section className="detail-hero"><div><span className={`lead-status ${slug(form.status)}`}>{statusLabel(form.status)}</span><p>{form.company} • {form.role||t("leads.roleMissing")}</p></div><strong>{priorityLabel(form.priority)}</strong></section>
   <div className="detail-grid"><article><span>{t("leads.contact")}</span><strong>{form.email||t("leads.noEmail")}</strong><p>{form.phone||t("leads.noPhone")}</p></article><article><span>{t("leads.location")}</span><strong>{form.city||"—"}</strong><p>{form.country}</p></article><article><span>{t("leads.owner")}</span><strong>{form.owner}</strong><p>{t("leads.originLabel",{source:sourceLabel(form.source)})}</p></article><article><span>{t("leads.nextAction")}</span><strong>{form.nextAction||t("leads.notDefined")}</strong><p>{t("leads.lastContactLabel",{date:form.lastContact||"—"})}</p></article></div>
   <section className="relationship-card"><p className="eyebrow">{t("leads.commercialInterest")}</p><h3>{form.interest||t("leads.notSpecified")}</h3><p>{form.notes||t("leads.noNotes")}</p></section>
   <section className="partner-insight"><div className="insightMark">N</div><div><p className="eyebrow light">{t("leads.insight")}</p><h3>{form.status==="Perdido"?t("leads.insightLost"):form.status==="Convertido"?t("leads.insightConverted"):t("leads.insightActive")}</h3><p>{form.nextAction?t("leads.recommendedAction",{action:form.nextAction}):t("leads.defineAction")}</p></div></section>
