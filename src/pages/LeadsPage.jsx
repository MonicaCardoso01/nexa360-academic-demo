...
function LeadModal({item,mode,onClose,onSave,onDelete,onConvert,canEdit,t,language}){
 ...
 return <div className="modal-backdrop"><section className="partner-modal lead-modal">
  ...
  {editing?<form className="partner-form" onSubmit={submit}>
   ...
   <section className="partner-insight"><BrandSymbol className="insightMark" /><div><p className="eyebrow light">{t("leads.insight")}</p><h3>{form.status==="Perdido"?t("leads.insightLost"):form.status==="Convertido"?t("leads.insightConverted"):t("leads.insightActive")}</h3><p>{displayForm.nextAction?t("leads.recommendedAction",{action:displayForm.nextAction}):t("leads.defineAction")}</p></div></section>
